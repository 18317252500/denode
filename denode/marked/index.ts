...
class Renderer {
    ...
    tablecell(content: string, { header, align }: any) {
...
function escape(html: string, encode?: boolean): any {
    if (encode) {
        if (escape.escapeTest.test(html)) {
            return html.replace(escape.escapeReplace, (ch: any) => (escape.replacements as any)[ch]);
        }
    } else {
        if (escape.escapeTestNoEncode.test(html)) {
            return html.replace(escape.escapeReplaceNoEncode, ch => (escape.replacements as any)[ch]);
        }
    }
...
function marked(src: string, optOrCallback: MarkedOptions | MarkedCallback): string;
