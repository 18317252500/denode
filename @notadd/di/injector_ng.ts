import { InjectFlags, OptionFlags, Provider } from './type.ts';
import { stringify, resolveForwardRef, staticError, catchInjectorError, NG_TEMP_TOKEN_PATH, NO_NEW_LINE } from './util.ts';
import { InjectionToken } from './injection_token.ts';
import { setCurrentInjector } from './injector_compatibility.ts';
import { ConstructorProvider, ExistingProvider, FactoryProvider, StaticClassProvider, StaticProvider, ValueProvider } from './type.ts';
import { INJECTOR_SCOPE } from './scope.ts';
import { getINgerDecorator, IClassDecorator } from '../decorator/index.ts';
import { InjectableMetadataKey, InjectableOptions, Optional, SkipSelf, Self, Inject } from './decorator.ts';
import { providerToStaticProvider } from './providerToStaticProvider.ts';
